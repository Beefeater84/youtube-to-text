...
- [x] 1. Миграция БД: retry_count, error_message, started_at + channel_id nullable + RPC function для grab job
- [x] 2. Web: Server Action для формы (oEmbed, find/create channel, insert transcript)
- [x] 3. Worker: инициализация проекта (package.json, tsconfig, config, db client, polling loop)
- [x] 4. Worker pipeline: fetch raw transcript (youtube-transcript)
- [x] 5. Worker pipeline: OpenAI cleanup + structuring (GPT-4o-mini, structured output)
- [x] 6. Worker pipeline: generate .md file по спеке transcript-format.md
- [x] 7. Worker pipeline: upload to Supabase Storage + update DB
- [x] 8. Docker: убрать Redis, обновить worker Dockerfile и docker-compose
- [x] 9. Обновить plan.md и data-storage.md

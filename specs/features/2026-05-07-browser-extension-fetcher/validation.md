...
- [x] `\d transcripts` shows new column `fetch_payload_path`.
- [x] `INSERT INTO transcripts (..., status='awaiting_browser_fetch') RETURNING id;` succeeds.
- [x] `INSERT … status='bogus'` fails with constraint violation (proves CHECK was rebuilt).
- [x] `SELECT bucket_id FROM storage.buckets WHERE id='fetch-payloads';` returns one row.
